...
<script type="text/javascript" src="http://103.42.31.90/public/Admin/js/adminModule.js"></script>
					...
					<div class="row">
						<div class="col-xs-12">
							<table id="simple-table" class="table table-striped table-bordered table-hover">
								...
								<tbody>
								<?php if($mList != ''): if(is_array($mList)): $i = 0; $__LIST__ = $mList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr id="module_<?php echo ($list['id']); ?>">
										<td><?php echo ($list['title']); ?></td>
										<td><?php echo ($list['code']); ?></td>
										<td><?php echo ($list['note']); ?></td>
										<td><?php echo ($isCate[$list['is_cate']]); ?></td>
										<td><?php echo ($statusList[$list['status']]); ?></td>
										<td><?php echo date('Y-m-d h:i:s', $list['ctime']);;?></td>
										<td>
											<div class="hidden-sm hidden-xs">
												<button class="btn btn-xs btn-info editModuleBox" moduleId="<?php echo ($list['id']); ?>" title="修改">
													<i class="ace-icon fa fa-pencil bigger-120"></i>
												</button>

												<button class="btn btn-xs btn-danger delModule" moduleId="<?php echo ($list['id']); ?>" moduleCode="<?php echo ($list['code']); ?>" title="删除">
													<i class="ace-icon fa fa-trash-o bigger-120"></i>
												</button>
												<?php if($list['is_ext'] == 0): ?><button class="btn btn-xs btn-success addField" moduleId="<?php echo ($list['id']); ?>" moduleCode="<?php echo ($list['code']); ?>" title="字段管理">
													<i class="ace-icon fa fa-file-code-o bigger-120"></i>
												</button><?php endif; ?>
											</div>
										</td>
