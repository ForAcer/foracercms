...
date_default_timezone_set('PRC'); // UTC: 标准时间，RPC: People's republic of China（中华人民共和国）
...
define('IS_HTTPS', (isset($_SERVER['HTTPS']) && strtolower($_SERVER['HTTPS']) != 'off') ? true : false);
...
{
	$siteRoot = dirname(_PHP_FILE_);
	define('SITE_ROOT', (($siteRoot == '/' || $siteRoot == '\\') ? '' : rtrim($siteRoot, '/')));
}

// 系统常量定义
define('SITE_DOMAIN', strip_tags($_SERVER['HTTP_HOST']));
define('SITE_URL', (IS_HTTPS ? 'https:' : 'http:') . '//' . SITE_DOMAIN . SITE_ROOT);
define('UPLOAD_URL', SITE_URL . '/data/upload');
define('PUBLIC_URL', SITE_URL . '/public');
define('PUBLIC_PATH', SITE_PATH . '/public');

// 静态文件PATH/URL
define('APP_STATIC_PATH', PUBLIC_PATH . '/static');
define('APP_STATIC_URL', PUBLIC_URL . '/static');

// 静态文件PATH/URL
define('COMMON_STATIC_PATH', PUBLIC_PATH . '/static/common');
define('COMMON_STATIC_URL', PUBLIC_URL . '/static/common');
