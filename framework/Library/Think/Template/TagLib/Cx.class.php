...
class Cx extends TagLib {
	...
	protected $tags   =  array(
		...
		'phpclist'  => array('attr'=>'id,code,number,type,sort', 'close'=> 1),
		'phpmlist'  => array('attr'=>'id,code,number,type,sort', 'close'=> 1),
		'phpcode'   => array('attr'=>'id,code', 'close'=> 1),
		'catelist'  => array('attr'=>'id,code', 'close'=> 1),
		'navlist'   => array('attr'=>'id', 'close'=>1),
		'phpprenext' => array('attr'=>'cate,list,id', 'close'=>1),
	);

	//获取当前主题前后主题
	public function _phpprenext($tag, $content)
	{
		$cateId = $tag['cate'];
		$listId = $tag['list'];
		$id = $tag['id'];
		...
		$str = '<?php ';
		$str .=	'$frontInfo = M("List")->field("id,cate_id,title")->where(array("cate_id" => "'.$cateId.'", "id"=>array("lt", "'.$listId.'")))->order(array("id"=> "desc"))->limit(1)->find();';
		$str .= '$afterInfo = M("List")->field("id,cate_id,title")->where(array("cate_id"=> "'.$cateId.'", "id"=>array("gt", "'.$listId.'")))->order(array("id"=> "desc"))->limit(1)->find();';
		$str .= '$data["0"] = $frontInfo;';
		$str .= '$data["1"] = $afterInfo;';
		$str .= 'foreach ($data as $key=>$'.$id.'){ ?>';
	...
	public function _navlist($tag, $content)
	{
		...
		$str = '<?php ';
		$str .='$getParentNavList = M("Nav")->where(array("parent_id"=>0,"is_del"=>0))->select();';
		$str .='$getAllNavList = M("Nav")->where(array("is_del"=>0))->select();';
		$str .='$data = array();';
		$str .='foreach ($getAllNavList as $v )';
		$str .='{';
		$str .='foreach ($getParentNavList as $key=>$value)';
		$str .='{';
		$str .='if($value["id"] == $v["parent_id"])';
		$str .='{';
		$str .='$getParentNavList[$key]["sonlist"][] = array(';
		$str .='"id" => $v["id"],';
		$str .='"module_id" => $v["module_id"],';
		$str .='"title" => $v["title"],';
		$str .='"link_url"=> $v["link_url"],';
		$str .=');';
		$str .='}';
		$str .='}';
		$str .='}';
		$str .= 'foreach ($getParentNavList as $key=>$'.$id.'){ ?>';
		$str .= $this->tpl->parse($content);
		$str .= '<?php }?>';
		return $str;
	}
	...
	public function _catelist($tag, $content)
	{
		...
		$str = '<?php ';
		$str .='$parentCateInfo = M("Cate")->where(array("code"=>  '.$code.', "is_del"=>0))->find();';
		$str .='$cWhere = array("parent_id"=> $parentCateInfo["id"], "is_del"=>0);';
		$str .='$sonlist = M("Cate")->where($cWhere)->order(array("sort"=>"asc"))->select();';
		$str .= 'foreach ($sonlist as $key=>$'.$id.'){ ?>';
		$str .= $this->tpl->parse($content);
		$str .= '<?php }?>';
		return $str;
	}
	...
	//根据单项标识串 获取对应的内容列表
	public function _phpcode($tag, $content)
	{
		$code = $tag['code'];
		$id = $tag['id'];

		$str = '<?php ';
		$str .= '$copyrightInfo[0] = M("Copyright")->where(array("code"=> "'.$code.'","is_del"=>0))->find();';
		$str .= 'foreach ($copyrightInfo as $key=>$'.$id.'){ ?>';
		$str .= $this->tpl->parse($content);
		$str .= '<?php }?>';
	...
	//根据模块标识串 获取对应模块下的内容列表
	public function _phpmlist($tag, $content)
	{
		$code = $tag['code'];
		$id = $tag['id'];
		$number = !empty($tag['number']) ? $tag['number'] : 10;
		$type = !empty($tag['type']) ? $tag['type'] : "all";
		$sort = !empty($tag['sort']) ? $tag['sort'] : "desc";

		$str  = '<?php ';
		$str .='$moduleWhere = array("code"=> "'.$code.'", "is_del"=>0);';
		$str .='$getModuleInfo = M("Module")->where($moduleWhere)->find();';
		$str .='if("'.$type.'" == "all")';
		$str .= '{';
		$str .=	'$moduleWhere1 = array("module_id"=> $getModuleInfo["id"], "is_del"=> 0);';
		$str .=	'$listArrInfo = M("List")->where($moduleWhere1)->order(array("is_top"=> "desc", "sort"=> "'.$sort.'", "ctime"=> "'.$sort.'"))->limit('.$number.')->select();';
		$str .= '}';
		$str .='elseif("'.$type.'" == "common")';
		$str .= '{';
		$str .=	'$moduleWhere2 = array("module_id"=> $getModuleInfo["id"], "is_top"=> 0, "is_best"=> 0, "is_vouch"=>0, "is_del"=> 0);';
		$str .=	'$listArrInfo = M("List")->where($moduleWhere2)->order(array("is_top"=> "desc", "sort"=> "'.$sort.'", "ctime"=> "'.$sort.'"))->limit('.$number.')->select();';
		$str .= '}';
		$str .= 'else';
		$str .= '{';
		$str .= '$moduleWhere3 = array("module_id"=> $getModuleInfo["id"], "'.$type.'" => 1, "is_del"=> 0);';
		$str .= '$listArrInfo = M("List")->where($moduleWhere3)->order(array("sort"=> "'.$sort.'", "ctime"=> "'.$sort.'"))->limit('.$number.')->select();';
		$str .= '}';
		$str .= '$listIds = getSubByKey($listArrInfo, "id");';
		$str .= 'if(!empty($listIds))';
		$str .= '{';
		$str .= '$listExtArrInfo = M("ListExt")->where(array("id"=> array("IN", $listIds)))->select();';
		$str .= '}';
		$str .= '$moduleFieldList = M("ModuleField")->field("module_id,field_code,field_type")->where(array("module_id"=> $getModuleInfo["id"], "field_type"=>"images", "is_del"=> 0))->order(array("sort"=>"desc"))->find();';
		$str .= 'if(!empty($listExtArrInfo))';
		$str .= '{';
		$str .= 'foreach($listExtArrInfo as $value)';
		$str .= '{';
		$str .= 'foreach($listArrInfo as $key=> $v)';
		$str .= '{';
		$str .= 'if($value["id"] == $v["id"])';
		$str .= '{';
		$str .= 'if($value["field"] == $moduleFieldList["field_code"])';
		$str .= '{';
		$str .= '$value["content"]  = explode(",", $value["content"]);';
		$str .= '}';
		$str .= '$listArrInfo[$key][$value["field"]] =  $value["content"];';
		$str .= '}';
		$str .= '}';
		$str .= '}';
		$str .= '}';
		$str .= 'foreach ($listArrInfo as $key=>$'.$id.'){ ?>';
		$str .= $this->tpl->parse($content);
		$str .= '<?php }?>';
	...
	//根据类标识串 获取对应分类主题内容
	public function _phpclist($tag, $content)
	{
		$code = $tag['code'];
		$id = $tag['id'];
		$number = !empty($tag['number']) ? $tag['number'] : 10;
		...
		$str  = '<?php ';
		$str .= '$cateWhere = array("code" => "'.$code.'", "is_del"=> "0");';
		$str .= '$getCateInfo = M("Cate")->where($cateWhere)->find();';
		$str .= 'if("'.$type.'" == "all")';
		$str .= '{';
		$str .= '$listWhere1 = array("cate_id"=> $getCateInfo["id"], "is_del"=> 0);';
		$str .= '$listArrInfo = M("List")->where($listWhere1)->order(array("is_top"=> "desc", "sort"=> "'.$sort.'", "ctime"=> "'.$sort.'"))->limit('.$number.')->select();';
		$str .= '}';
		$str .= 'elseif("'.$type.'" == "common")';
		$str .= '{';
		$str .= '$listWhere2 = array("cate_id"=> $getCateInfo["id"],"is_top"=> 0, "is_best"=> 0, "is_vouch"=>0, "is_del"=> 0);';
		$str .= '$listArrInfo = M("List")->where($listWhere2)->order(array("is_top"=> "desc", "sort"=> "'.$sort.'", "ctime"=> "'.$sort.'"))->limit('.$number.')->select();';
		$str .= '}';
		$str .='else';
		$str .= '{';
		$str .= '$listWhere3 = array("cate_id"=> $getCateInfo["id"], "'.$type.'" => 1, "is_del"=> 0);';
		$str .= '$listArrInfo = M("List")->where($listWhere3)->order(array("sort"=> "'.$sort.'", "ctime"=> "'.$sort.'"))->limit('.$number.')->select();';
		$str .= '}';
		$str .= '$listIds = getSubByKey($listArrInfo, "id");';
		$str .= 'if(!empty($listIds)){';
		$str .= '$listExtArrInfo = M("ListExt")->where(array("id"=> array("IN", $listIds)))->select();';
		$str .= '}';
		$str .= '$moduleFieldList = M("ModuleField")->field("module_id,field_code,field_type")->where(array("module_id"=> $getCateInfo["module_id"], "field_type"=>"images", "is_del"=> 0))->order(array("sort"=>"desc"))->find();';
		$str .= 'if(!empty($listExtArrInfo))';
		$str .= '{';
		$str .='foreach($listExtArrInfo as $value)';
		$str .= '{';
