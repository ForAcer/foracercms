...
function U($url = '', $params = false, $redirect = false)
{
	...
	{
		// 载入路由
		$routerRuler = C('router');
		$routerKey = $app . '/' . $mod . '/' . $act;

		// 路由命中
		if (isset($routerRuler[$routerKey]))
		{
			// 填充路由参数
			if (false == strpos($routerRuler[$routerKey], '://')) { // 注意是 ==，而不是 ===
				$siteUrl = SITE_URL . $routerRuler[$routerKey];
			}
			else
			{
				$siteUrl = $routerRuler[$routerKey];
			}
			...
			if ($queryStr)
			{
				...
				foreach ($args as $k => $v)
				{
					if (strpos($siteUrl, '[' . $k . ']'))
					{
						$siteUrl = str_replace('[' . $k . ']', $v, $siteUrl);
					}
					else
					{
						$other[$k] = $v;
				...
				{
					$siteUrl .= '?' . http_build_query($other);
				}
			}
		}
		else // Rest风格
		{
			$siteUrl = SITE_URL . '/' . $app . '/' . $mod . '/' . $act;
			$queryStr && ($siteUrl .=  '?' . $queryStr);
		}
	}
	else
	{
		// 组合默认路径
		$siteUrl = SITE_URL . '/index.php?app=' . $app . '&mod=' . $mod . '&act=' . $act;
		$queryStr && ($siteUrl .=  '&' . $queryStr);
	}
...
function get_page_url()
{
	...
	$phpSelf = $_SERVER['PHP_SELF'] ? safe_replace($_SERVER['PHP_SELF']) : safe_replace($_SERVER['SCRIPT_NAME']);
	$pathInfo = isset($_SERVER['PATH_INFO']) ? safe_replace($_SERVER['PATH_INFO']) : '';
	$relateUrl = isset($_SERVER['REQUEST_URI']) ? safe_replace($_SERVER['REQUEST_URI']) : $phpSelf . (isset($_SERVER['QUERY_STRING']) ? '?' . safe_replace($_SERVER['QUERY_STRING']) : $pathInfo);

	return $protocal . (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '') . $relateUrl;
}
...
function friendly_date($sTime, $type = 'normal', $alt = 'false')
{
	...
	elseif ($type == 'ymdhis') //添加年月日时分秒的类型
	{
		return date("Y-m-d H:i:s", $sTime);
	}
	else
...
/**
 * 格式化数字显示，超过5位用万显示，超过9位用亿显示
 * @param int $formatNum 数字
 * @return string 友好数字显示
 */
function format_number($formatNum){
	$num = floatval($formatNum);
	return ($num/100000000 > 1) ? sprintf( "%.1f ", ($num/100000000)) . '亿' : (($num/10000 > 1) ? sprintf( "%.1f ",($num/10000)) . '万' : $num);
}

/**
 * 我的记录数值判断，五位数以上用万表示
 */
function numFormat($formatNum){
	$num = intval($formatNum);
	return ($num/100000000 > 1)
		? '<strong>' . sprintf( "%.1f ", ($num/100000000)) . '<b class="font"><b>亿</b></b></strong>'
		: (
		($num/10000 > 1)
			? '<strong>' . sprintf( "%.1f ",($num/10000)) . '<b class="font"><b>万</b></b></strong>'
			:'<strong>' . $num . '</strong>'
		);
}
 ...
 * @return string
 */
function get_short($str, $length = 20, $ext = '')
{
 ...
 * @return boolean
 */
function validate_match($value, $rule)
{
	$validate = array(
		'require' => '/\S+/',
		'email' => '/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/',
		...
		'phone_mobile' => '/(^(([0\+]\d{2,3}-)?(0\d{2,3})-)(\d{7,8})(-(\d{3,}))?$)|(^0{0,1}1[3|4|5|6|7|8|9][0-9]{9}$)/',
	);
 ...
 * @return string
 */
function second_to_hms($msec)
{
	$hour = intval($msec/3600);
	$mint = intval(($msec%3600)/60);
	$second = intval(($msec%3600)%60);
	$data_format_string = sprintf("%02d", $hour).":".sprintf("%02d", $mint).":".sprintf("%02d", $second);
	return $data_format_string;
}

/**
 * 验证码验证函数
 * @param $code 用户输入的验证码
 * @param string $id 用户设置的表示窜
 * @return bool
 */
...
/**
 * 获取 get, post, cookie 当中的某个变量
 * 当一个变量被设置为null时，系统就不会报Undefined错误
 *
 * @param string $key key 值
 * @param string $type 类型
 */
function get_gpc($key, $type = 'GP')
{
 ...
 * @param int $size 每页大小
 * @param int $ajaxDiv 以ajax方式返回数据显示区域的id
 * @param int $ajaxDiv 以ajax方式返回数据的格式，HTML、JSON、XML
 * @return mixed
 */
function get_page_bar($total, $page = 1, $size = 20, $ajaxDiv = '', $toDiv = '',
	$pageUrl = '', $callback = null,
	$waitId = null, $loading = null, $display = null,
	$columnPages = 5, $maxPages = 0
)
{
	if (empty($total) || intval($total) <= 0)
	{
		return "";
	}
	$pageSize = empty($size) || intval($size) <= 0 ? 20 : intval($size);
	$curPage = empty($page) || intval($page) <= 0 ? 1 : intval($page);
	$page = new \Think\Page($total, $pageSize, $curPage, $ajaxDiv,
		$toDiv, $pageUrl, $columnPages, $maxPages,
		$waitId, $loading, $display, $callback);

	return $page->show();
}


function msg_url($data)
{
	if(!empty($data))
	{
		$cateId = $data['cate_id'];
	...
	return url('List/msg', array('cid'=>$cateId, 'id'=>$listId));
}
...
function list_url($data)
{
	...
	return url('List/cate', array('id'=>$cateId));
}
