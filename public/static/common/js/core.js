...
var _core = function() {
	...
	this._loadCss = function() {
		...
		var tempMethod = function(url) {
			...
			if(flag == 0) {
				// 未载入过
				temp[temp.length] = url;
				var css = '<link href="'+COMMON_STATIC_URL+'/js/tbox/box.css" rel="stylesheet" type="text/css">';
				$('head').append(css);
			}
		};
		// 返回内部包函数,供外部调用并可以更改temp的值
	...
	this._rcalendar = function() {
		...
		var tempMethod = function(t, m, r) {
			...
			if(temp == 0) {
				// JQuery的ajax载入文件方式，如果有样式文件，同理在此引入相关样式文件
				$.getScript(COMMON_STATIC_URL+'/js/rcalendar.js', function() {
					rcalendar(t, m, r);
				});
			} else {
				rcalendar(t, m, r);
		...
		var _c = function() {
			if(_imgHtml == '') {
				$.post(U('public/Feed/getSmile'), {}, function(data) {
					for(var k in data) {
						_imgHtml += '<a href="javascript:void(0)" title="'+data[k].title+'" onclick="core.face.face_chose(this)";><img src="'+ COMMON_STATIC_URL +'/image/expression/'+data[k].type+'/'+ data[k].filename +'" width="24" height="24" /></a>';
					}
...
core.plugInit = function() {
	if(arguments.length > 0) {
		...
		var file = COMMON_STATIC_URL + '/js/plugins/core.' + arguments[0] + '.js';
		core.loadFile(file, back);
	}
};
//与上面方法类似 只不过可以自己写回调函数（不主动执行init）
core.plugFunc = function(plugName,callback){
	var file = COMMON_STATIC_URL+'/js/plugins/core.'+plugName+'.js';
...
var ui = {
	 ...
	 * @param integer top 提示框距离浏览器顶部高度 (px)
	 * @return void
	 */
	showMessage: function(message, error, lazytime, top) {
		...
		if("undefined" == typeof(top) || parseInt(top)<0) {
			top  = $(window).scrollTop() + ($(window).height()-_h)/2;
		} else {
			top = parseInt(top);
		}
		// 添加弹窗样式与动画效果（出现）
	 ...
	 * @return void
	 */
	success: function(message, time, top) {
		var t = "undefined" == typeof(time) ? 3 : time;
		ui.showMessage(message, 0, t, top);
	},
	 ...
	 * @return void
	 */
	error: function(message, time, top) {
		var t = "undefined" == typeof(time) ? 3 : time;
		ui.showMessage(message, 1, t, top);
	},
	...
	box: {
		...
		inited: false,
		init: function(title, callback, options) {
			...
			//var url = COMMON_STATIC_URL+'/js/tbox/box.css';
			//core.loadCss(url);
			// 添加头部
			...
			if( "undefined"!=typeof(options) ) {
				if( "undefined"!=options.top && options.top>0) this.moveToY( parseInt(options.top) );
				if( "undefined"!=options.left && options.left>0) this.moveToX( parseInt(options.left) );
			} else {
				this.center();
			}
			var show = function(){
		...
		setcontent: function(content) {
			...
			//evalScript(content);
			//ajaxInnerhtml($$('layer-content'), content);
		},
		 ...
		 * @return void
		 */
		load:function(requestUrl,title,callback,requestData,type, options) {
			this.init(title,callback, options);
			...
			jQuery.ajax({url:requestUrl,
				...
				success:function(html){
					obj.setcontent(html);
					if( "undefined"!=typeof(options) ) {
						if( "undefined"!=options.top && options.top>0) obj.moveToY( parseInt(options.top) );
						if( "undefined"!=options.left && options.left>0) obj.moveToX( parseInt(options.left) );
					} else {
						obj.center();
					}
				}
		...
		/**
		 * Window对象 信息 left,top,width,height
		 * @return void
		 */
		_viewport: function() {
